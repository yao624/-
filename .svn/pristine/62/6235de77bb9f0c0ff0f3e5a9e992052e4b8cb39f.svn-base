...
class MaskPlugin implements IPluginTempl {
  ...
  setCoverMask(hack = false) {
    ...
    // 检查 workspace 尺寸是否有效
    if (!this.workspace.width || !this.workspace.height ||
        this.workspace.width <= 0 || this.workspace.height <= 0) {
      console.warn('Workspace 尺寸无效，跳过设置蒙版');
      return;
    }
  ...
  initMask(needBindLoadJSON = true) {
    ...
    if (!this.workspace.width || !this.workspace.height ||
        this.workspace.width <= 0 || this.workspace.height <= 0) {
      console.warn('Workspace 尺寸无效，延迟初始化蒙版', {
        width: this.workspace.width,
        height: this.workspace.height
      });
      // 延迟尝试初始化
      setTimeout(() => this.initMask(needBindLoadJSON), 100);
      return;
    }
