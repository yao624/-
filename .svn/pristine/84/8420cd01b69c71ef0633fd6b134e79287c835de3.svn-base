...
class FacebookFetchAdInsights implements ShouldQueue
{
...
//        }
    ...
    private function saveAdInsights(Collection $resp)
    {
        $data = collect($resp['data']);

        $adIds = collect();

        $data->each(function ($insightData) use ($adIds) {
            $add_to_cart = null;
            $purchase = null;
            $lead = null;
            $comment = null;
        ...
        // 更新 FbAd 的 update_at 字段
        if ($adIds->isNotEmpty()) {
            FbAd::whereIn('source_id', $adIds->unique())->update(['updated_at' => now()]);
        }
    }
    ...
    {
        // Log failure
        Log::error('FacebookFetchAdInsights Job failed: ' . $exception->getMessage());
        $msg = "failed to fetch ad insights: ad acc {$this->sourceID}";
        Telegram::sendMessage($msg);
    }
