<script lang="ts" setup>
import align from '@/components/fabric-editor/align.vue';
import centerAlign from '@/components/fabric-editor/centerAlign.vue';
import flip from '@/components/fabric-editor/flip.vue';

import clone from '@/components/fabric-editor/clone.vue';
import hide from '@/components/fabric-editor/hide.vue';
import group from '@/components/fabric-editor/group.vue';
import lock from '@/components/fabric-editor/lock.vue';
import edit from '@/components/fabric-editor/edit.vue';
import dele from '@/components/fabric-editor/del.vue';

import bgBar from '@/components/fabric-editor/bgBar.vue';
import setSize from '@/components/fabric-editor/setSize.vue';
import replaceImg from '@/components/fabric-editor/replaceImg.vue';
import filters from '@/components/fabric-editor/filters.vue';
import imgStroke from '@/components/fabric-editor/imgStroke.vue';
// import elementData from '@/components/elementData.vue';
// 右侧组件
// import attribute from '@/components/attribute.vue';
import attributePostion from '@/components/fabric-editor/attributePostion.vue';
import attributeId from '@/components/fabric-editor/attributeId.vue';
import attributeShadow from '@/components/fabric-editor/attributeShadow.vue';
import attributeBorder from '@/components/fabric-editor/attributeBorder.vue';
import attributeRounded from '@/components/fabric-editor/attributeRounded.vue';
import attributeFont from '@/components/fabric-editor/attributeFont.vue';
import attributeTextFloat from '@/components/fabric-editor/attributeTextFloat.vue';
import attributeTextContent from '@/components/fabric-editor/attributeTextContent.vue';
import attributeColor from '@/components/fabric-editor/attributeColor.vue';
import attributeBarcode from '@/components/fabric-editor/attributeBarcode.vue';
import attributeQrCode from '@/components/fabric-editor/attributeQrCode.vue';
import cropperImg from '@/components/fabric-editor/cropperImg.vue';
import clipImage from '@/components/fabric-editor/clipImage.vue';
import dynamicVariablePanel from '@/components/fabric-editor/dynamicVariable/index.vue'; 
// hooks
import useSelectListen from '@/hooks/useSelectListen';
import { ref, inject } from 'vue';



const canvasEditor: any = inject('canvasEditor');

const { mixinState } = useSelectListen(canvasEditor);

const attrBarShow = ref(true);

// 属性面板开关
const switchAttrBar = () => {
  attrBarShow.value = !attrBarShow.value;
};
</script>

<template>
  <!-- 属性区域 380-->
  <div class="right-bar" v-show="attrBarShow">
    <div style="padding-top: 10px">
      <!-- 未选择元素时 展示背景设置 -->
      <div v-show="!mixinState.mSelectMode">
        <set-size></set-size>
        <bg-bar></bg-bar>
      </div>

      <!-- 多选时展示 -->
      <div v-show="mixinState.mSelectMode === 'multiple'">
        <!-- 分组 -->
        <group></group>
        <!-- <Divider plain></Divider> -->
        <!-- 组对齐方式 -->
        <align></align>
        <!-- 居中对齐 -->
        <center-align></center-align>
      </div>

      <div v-show="mixinState.mSelectMode === 'one'" class="attr-item-box">
        <!-- <h3>快捷操作</h3> -->
        <!-- 分组 -->
        <group></group>
        <!-- <Divider plain></Divider> -->
        <Divider plain orientation="left">
          <h4>快捷操作</h4>
        </Divider>
        <div class="bg-item" v-show="mixinState.mSelectMode">
          <lock></lock>
          <dele></dele>
          <clone></clone>
          <hide></hide>
          <edit></edit>
        </div>
        <!-- <Divider plain></Divider> -->
        <!-- 居中对齐 -->
        <center-align></center-align>
        <!-- 替换图片 -->
        <replaceImg></replaceImg>
        <!-- 裁剪 -->
        <cropperImg></cropperImg>
        <!-- 图片裁切 -->
        <clip-image></clip-image>
        <!-- 翻转 -->
        <flip></flip>
        <!-- 条形码属性 -->
        <attributeBarcode></attributeBarcode>
        <!-- 二维码 -->
        <attributeQrCode></attributeQrCode>
        <!-- 图片滤镜 -->
        <filters></filters>
        <!-- 图片描边 -->
        <imgStroke />
        <!-- 颜色 -->
        <attributeColor></attributeColor>
        <!-- 字体属性 -->
        <attributeFont></attributeFont>
        <!-- 字体小数点 -->
        <attributeTextFloat></attributeTextFloat>
        <!-- 文字内容  -->
        <attribute-text-content></attribute-text-content>
        <!-- 位置信息 -->
        <attributePostion></attributePostion>
        <!-- 阴影 -->
        <attributeShadow></attributeShadow>
        <!-- 边框 -->
        <attributeBorder></attributeBorder>
        <!-- 圆角 -->
        <attributeRounded></attributeRounded>
        <!-- 关联数据 -->
        <attributeId></attributeId>

        <!-- 动态变量配置 -->
        <Divider plain orientation="left">
          <h4>动态变量</h4>
        </Divider>
        <dynamicVariablePanel></dynamicVariablePanel>

        <!-- 新增字体样式使用 -->
        <Button @click="canvasEditor.getFontJson()" size="small">获取元素数据</Button>
      </div>
    </div>
    <!-- <attribute v-if="state.show"></attribute> -->
  </div>
  <!-- 右侧关闭按钮 -->
  <div
    :class="`close-btn right-btn ${attrBarShow && 'right-btn-open'}`"
    @click="switchAttrBar"
  ></div>
</template>

<style lang="less" scoped>
// 右侧容器
.right-bar {
  width: 304px;
  height: 100%;
  padding: 10px;
  overflow-y: auto;
  background: #fff;
  border-radius: 8px;
}

// 属性面板样式
:deep(.attr-item) {
  position: relative;
  margin-bottom: 12px;
  height: 40px;
  padding: 0 10px;
  background: #f6f7f9;
  border: none;
  border-radius: 4px;
  display: flex;
  align-items: center;
  .ivu-tooltip {
    text-align: center;
    flex: 1;
  }
}

// 快捷操作按钮组样式
:deep(.bg-item) {
  width: 100%;
  padding: 5px;
  display: flex;
  flex: 1;
  justify-content: space-between;
  border-radius: 5px;
  background: #f6f7f9;

  .ivu-tooltip {
    flex: 1;
  }

  .ivu-btn {
    flex: 1;
    margin: 0 2px;
  }
}

// 关闭按钮
.close-btn {
  width: 20px;
  height: 64px;
  cursor: pointer;
  background-image: url(data:image/png;base64,);
  background-repeat: no-repeat;
  background-size: cover;
  background-position: 50%;
  position: absolute;
  right: -20px;
  z-index: 1;
  top: 50%;
  margin-top: -10px;

  &.right-btn {
    background-image: url(data:image/png;base64,);
    transform: rotateY(180deg);
    right: 0px;
  }

  &.right-btn-open {
    background-image: url(data:image/png;base64,);
    right: 304px;
  }
}
</style>
