  ...
  <page-container :title="t('pages.links.title')">
    <a-card mb-4>
      <a-row :gutter="[15, 0]" v-if="expand">
        <a-col :span="6">
          <a-form-item :label="t('pages.links.link')">
            <a-input v-model:value="pagination.link" />
          </a-form-item>
        </a-col>
        <a-col :span="6">
    ...
    <a-modal v-if="open" v-model:open="open" :title="dialogTitle" @ok="handleOk" :width="800">
      ...
      >
        <a-form-item name="link" :label="t('pages.links.link')" :rules="urlRules">
          <a-input v-model:value="dynamicValidateForm.link" />
        </a-form-item>
        ...
        <a-form-item name="tags" :label="'Tags'">
          <a-select
            v-model:value="dynamicValidateForm.tags"
            :options="tagOptions"
            mode="tags"
...
import { addTagsOneApi, deleteTagsOneApi } from '@/api/networks';
import { queryLinksApi, addLinksOneApi, deleteLinksOneApi, getLinksValidTags } from '@/api/links';
...
export default defineComponent({
  ...
  setup() {
    ...
    const urlRules = [
      ...
      {
        pattern: new RegExp(/^(https?|http?|socks5):\/\/[^\s/$.?#].[^\s]*$/i),
    ...
    const deleteOne = (data: any) => {
      loading.value = true;
      deleteLinksOneApi(data.id).then(() => {
    ...
    const showModal = (data: any) => {
      if (data) {
        ...
        dynamicValidateForm.value.tags = tagNames;
      } else {
