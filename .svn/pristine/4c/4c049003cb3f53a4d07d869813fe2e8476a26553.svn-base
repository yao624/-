      ...
      <a-dropdown v-if="batchOperationChecked">
        <template #overlay>
          <a-menu>
            ...
            <a-menu-item>{{ t('复制广告') }}</a-menu-item>
            <a-menu-item>{{ t('替换素材') }}</a-menu-item>
            <a-menu-item>{{ t('定时开关') }}</a-menu-item>
            <a-menu-item>{{ t('删除') }}</a-menu-item>
            <a-menu-item>{{ t('修改名称') }}</a-menu-item>
            <a-menu-item>{{ t('编辑标签') }}</a-menu-item>
            <a-menu-item>{{ t('修改网址参数') }}</a-menu-item>
          </a-menu>
        </template>
...
import { getAdReportData } from '../mock-data';
...
const columns = ref([
  { title: t('广告名称'), dataIndex: 'adName', key: 'adName', sorter: true },
...
const loadData = async () => {
  loading.value = true;
  try {
    const result = await getAdReportData({
