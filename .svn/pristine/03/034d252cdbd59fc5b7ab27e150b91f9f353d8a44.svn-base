...
class ActionRenameFbObject implements ShouldQueue
{
    ...
    private $object_type;
    private $object_name;
    public function __construct(string $item_source_id, $object_type, $object_name)
    {
    ...
    {
        Log::debug("--- start rename item {$this->object_type}: {$this->item_source_id}, to: {$this->object_name} ");
        ...
        try {
            ...
            $body['name'] = $this->object_name;
            Log::info("rename body:");
            Log::debug($body);
...
//            $fbAccount = $adAccount->fbAccounts()->where('token_valid', true)->firstOrFail();
            ...
            Log::debug("rename resp");
            Log::debug($resp);

            Log::debug("--- start rename item {$this->object_type}: {$this->item_source_id} ");
    ...
    {
        Log::error('Action rename failed: ' . $exception->getMessage());
    }
