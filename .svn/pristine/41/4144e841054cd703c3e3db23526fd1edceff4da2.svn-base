...
const sideWidth = 250; // 菜单宽度，确保菜单项文字完整显示
const collapsedWidth = 48;
const secondSideWidth = 160; // for leftmenu-layout (保留用于可能的未来需求)
...
export default function useMenuState(
  ...
  const menuWidth = computed(() => {
    ...
    // 修复：left 布局也使用 sideWidth，而不是 firstSideWidth
    const width: number = sideWidth; // 统一使用 sideWidth，确保菜单宽度生效
