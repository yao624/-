...
class FacebookFetchAdsetInsights implements ShouldQueue
{
...
//        }
        ...
        foreach ($dateRange as $currentDate) {
            ...
            if ($this->next) {
                FacebookFetchAdInsights::dispatch($this->fbAdAccountID, $this->dateStart, $this->dateStop, $this->fbAccountID, $this->next)
    ...
    private function saveAdsetInsights(Collection $resp)
    {
        $data = collect($resp['data']);

        $adsetIds = collect();

        $data->each(function ($insightData) use ($adsetIds) {
            $add_to_cart = null;
            $purchase = null;
            $lead = null;
            $comment = null;
            ...
            $adsetIds->push($insightData['adset_id']);

            FbAdsetInsight::query()->updateOrCreate(
                [
                    'account_id' => $insightData['account_id'],
                    'campaign_id' => $insightData['campaign_id'],
                    'adset_id' => $insightData['adset_id'],
                ...
                [
                    ...
                    'adset_name' => $insightData['adset_name'],
        ...
        // 更新 FbAdset 的 update_at 字段
        if ($adsetIds->isNotEmpty()) {
            FbAdset::whereIn('source_id', $adsetIds->unique())->update(['updated_at' => now()]);
        }
    }
    ...
    {
        // Log failure
        Log::error('FacebookFetchAdsetInsights Job failed: ' . $exception->getMessage());
        $msg = "failed to fetch adset insights: ad acc {$this->sourceID}";
        Telegram::sendMessage($msg);
    }
