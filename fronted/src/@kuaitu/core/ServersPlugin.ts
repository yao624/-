...
class ServersPlugin implements IPluginTempl {
  ...
  async loadJSON(jsonFile: string | object, callback?: () => void) {
    ...
    // 清理无效的 clipPath
    temp.objects = temp.objects.filter((item: any) => {
      // 过滤掉没有基本属性的对象
      if (!item.type) {
        console.warn('过滤掉没有 type 属性的对象:', item);
        return false;
      }

      // 清理无效的 clipPath
      if (item.clipPath) {
        const clipPath = item.clipPath;
        ...
        if (hasInvalidSize) {
          console.warn('清理 JSON 中的无效 clipPath:', {
            objectType: item.type,
            clipPathType: clipPath.type,
            clipPathSize: { width: clipPath.width, height: clipPath.height }
          });
          delete item.clipPath;
        }
      }
      ...
      this.canvas.loadFromJSON(jsonFile, () => {
        ...
        this.canvas.getObjects().forEach((obj: any) => {
          if (obj.clipPath) {
            ...
            if (hasInvalidSize) {
              console.warn('清理加载后对象的无效 clipPath:', obj.type);
        ...
        this.editor.hooksEntity.hookImportAfter.callAsync(jsonFile, () => {
          ...
          // 确保拖拽模式已关闭
          if (this.editor.endDring && typeof this.editor.endDring === 'function') {
            this.editor.endDring();
          }
