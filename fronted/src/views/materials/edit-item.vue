    ...
    >
      ...
      >
        <a-form-item name="name" :label="t('Name')" :rules="[{ required: true }]">
          <a-textarea v-model:value="modelRef.name" />
        </a-form-item>
...
import type { MaterialEditModel } from '@/utils/fb-interfaces';
import { editMaterialApi } from '@/api/materials';
...
export default defineComponent({
  ...
  props: {
    ...
    model: {
      type: Object as PropType<MaterialEditModel | null>,
      default: () => null,
    },
  },
  emits: ['ok', 'cancel'],
  setup(props, { emit }) {
    ...
    const modelRef = reactive<MaterialEditModel>({
      id: '',
      name: '',
      notes: '',
    });
    ...
    const handleOk = () => {
      ...
      validate().then(() => {
        ...
        loading.value = true;
        editMaterialApi(modelRef.id, params)
          .then(() => {
            emit('ok');
