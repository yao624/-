...
import { defineComponent, h, ref, toRefs, createTextVNode } from 'vue';
  ...
  setup(props, { slots, emit }) {
    ...
    const customBreadcrumbRender = ({ route, params, routes }: BreadcrumbItemRender) => {
      ...
      if (routes.indexOf(route) === routes.length - 1) {
        return h('span', null, [createTextVNode(breadcrumbName)]);
      }
      return h(
        RouterLink,
        { to: { path: route.path || '/', params } } as any,
        () => [createTextVNode(breadcrumbName)],
      );
    };
